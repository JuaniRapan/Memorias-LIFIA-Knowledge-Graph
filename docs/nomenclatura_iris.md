...
| Tema / Línea I+D (`tags`/`keywords`, sin tabla propia)                          | no tiene, se normaliza el string | `.../resource/tema/{tag-normalizado}`, y se usa solo si el tag no matchea con nada en CSO. Si matchea, la entidad se conecta directo a la IRI de CSO (no se crea un recurso propio ni ningún triple de equivalencia aparte: la URI del tema *es* la de CSO, no hay dos nodos que enlazar) |
